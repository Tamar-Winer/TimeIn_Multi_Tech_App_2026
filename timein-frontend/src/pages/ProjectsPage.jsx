...
export default function ProjectsPage() {
  ...
  const [editProject, setEditP]     = useState(null); // null | 'new' | obj
  const [editTask, setEditT]        = useState(null); // null | 'new' | obj
  ...
  const openNewProject = () => {
    setPF({ projectName:'', description:'', managerId:'', gitRepositoryName:'', gitRepositoryUrl:'', clickupSpaceId:'' });
    setEditP('new');
  };
  const openEditProject = (p) => {
    setPF({ projectName:p.project_name, description:p.description||'', managerId:String(p.manager_id||''), gitRepositoryName:p.git_repository_name||'', gitRepositoryUrl:p.git_repository_url||'', clickupSpaceId:p.clickup_space_id||'' });
    setEditP(p);
  };
  ...
  // ── task actions ──────────────────────────────────────────────
  const openNewTask = () => {
    setTF({ taskName:'', description:'', projectId:projectFilter||'', assignedUserId:'', priority:'medium', estimatedHours:'', dueDate:'', clickupTaskId:'' });
    setEditT('new');
  };
  const openEditTask = (t) => {
    setTF({ taskName:t.task_name, description:t.description||'', projectId:String(t.project_id), assignedUserId:String(t.assigned_user_id||''), priority:t.priority||'medium', estimatedHours:String(t.estimated_hours||''), dueDate:t.due_date?.slice(0,10)||'', clickupTaskId:t.clickup_task_id||'' });
    setEditT(t);
  };
      ...
      {tab === 'projects' && (
        <>
            ...
            <Card style={{ marginBottom:16, border:'2px solid #e0e7ff' }}>
              ...
              <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
                <div style={{ display:'flex', gap:12 }}>
                  <div style={{ flex:2 }}><label style={lbl}>שם הפרויקט *</label><input value={pF.projectName} onChange={e=>setPF(p=>({...p,projectName:e.target.value}))} style={inp} placeholder="שם הפרויקט" /></div>
                  <div style={{ flex:1 }}><label style={lbl}>מנהל</label>
                ...
                {editProject==='new' && (
                  <div style={{ display:'flex', gap:12 }}>
                    <div style={{ flex:1 }}><label style={lbl}>Git Repository</label><input value={pF.gitRepositoryName} onChange={e=>setPF(p=>({...p,gitRepositoryName:e.target.value}))} style={inp} placeholder="my-repo" /></div>
                    <div style={{ flex:1 }}><label style={lbl}>ClickUp Space ID</label><input value={pF.clickupSpaceId} onChange={e=>setPF(p=>({...p,clickupSpaceId:e.target.value}))} style={inp} placeholder="abc123" /></div>
                  </div>
                )}
          ...
          <Card>
            ...
            {projects.map(p => (
              <div key={p.id} style={{ display:'flex', alignItems:'center', gap:12, padding:'13px 0', borderBottom:'1px solid #f1f5f9' }}>
                <div style={{ width:10, height:10, borderRadius:'50%', background:S_COLOR[p.status]||'#94a3b8', flexShrink:0, marginTop:1 }} />
                ...
                <div style={{ fontSize:11, color:'#64748b', whiteSpace:'nowrap' }}>{p.manager_name||'—'}</div>
                {p.git_repository_name && <code style={{ fontSize:10, background:'#f1f5f9', padding:'2px 6px', borderRadius:4, whiteSpace:'nowrap' }}>{p.git_repository_name}</code>}
        ...
        <>
          ...
          {editTask && (
            <Card style={{ marginBottom:16, border:'2px solid #e0e7ff' }}>
              ...
              <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
                <div style={{ display:'flex', gap:12 }}>
                  <div style={{ flex:2 }}><label style={lbl}>שם המשימה *</label><input value={tF.taskName} onChange={e=>setTF(t=>({...t,taskName:e.target.value}))} style={inp} placeholder="שם המשימה" autoFocus /></div>
                  <div style={{ flex:1 }}><label style={lbl}>פרויקט *</label>
                ...
                <div style={{ display:'flex', gap:12 }}>
                  <div style={{ flex:1 }}><label style={lbl}>הקצה לעובד</label>
                  ...
                  <div style={{ flex:1 }}><label style={lbl}>עדיפות</label>
                  ...
                  <div style={{ flex:1 }}><label style={lbl}>שעות משוערות</label><input type="number" min="0.5" step="0.5" value={tF.estimatedHours} onChange={e=>setTF(t=>({...t,estimatedHours:e.target.value}))} style={inp} placeholder="8" /></div>
                </div>
                <div style={{ display:'flex', gap:12 }}>
                  <div style={{ flex:1 }}><label style={lbl}>תאריך יעד</label><input type="date" value={tF.dueDate} onChange={e=>setTF(t=>({...t,dueDate:e.target.value}))} style={inp}/></div>
                  <div style={{ flex:1 }}><label style={lbl}>ClickUp Task ID</label><input value={tF.clickupTaskId} onChange={e=>setTF(t=>({...t,clickupTaskId:e.target.value}))} style={inp} placeholder="CU-T001" /></div>
                </div>
                <div style={{ display:'flex', gap:10 }}>
          ...
          <Card>
            ...
            {tasks.map(t => {
              ...
              return (
                <div key={t.id} style={{ display:'flex', alignItems:'center', gap:10, padding:'12px 0', borderBottom:'1px solid #f1f5f9', fontSize:12 }}>
                  <div style={{ flex:1, minWidth:0 }}>
                    <div style={{ fontWeight:500, color:'#1e293b' }}>{t.task_name}</div>
                    <div style={{ color:'#94a3b8', fontSize:11, marginTop:2 }}>{t.project_name}</div>
                  </div>
                  <div style={{ color:'#64748b', whiteSpace:'nowrap' }}>{t.assigned_user_name || 'לא הוקצה'}</div>
                  <span style={{ fontSize:11, color:P_COLOR[t.priority], fontWeight:500, whiteSpace:'nowrap' }}>{P_LABEL[t.priority]}</span>
                  {t.due_date && <span style={{ fontSize:11, color:overdue?'#ef4444':'#94a3b8', whiteSpace:'nowrap' }} title={overdue?'חריגת תאריך':''}>{overdue?'⚠ ':''}{t.due_date?.slice(0,10)}</span>}
                  {t.estimated_hours && <span style={{ fontSize:11, color:'#64748b' }}>{t.estimated_hours}ש'</span>}
